...
type Config struct {
	...
	SealerAddress common.Address `toml:",omitempty"` // Address for sealing blocks (default = first account)
...
func (miner *Miner) Pending() (*types.Block, *state.StateDB) {
	...
	stateDb, err := miner.worker.chain.StateAt(head.Root)
	if err != nil {
		return nil, nil
	}
...
func (miner *Miner) GetSealingBlock(parent common.Hash, timestamp uint64, coinbase common.Address, random common.Hash, withdrawals types.Withdrawals, noTxs bool) (*types.Block, *big.Int, error) {
	return miner.worker.getSealingBlock(parent, timestamp, coinbase, random, withdrawals, noTxs)
}
