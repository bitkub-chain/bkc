...
func (w *worker) mainLoop() {
	...
	for {
		...
		case req := <-w.newWorkCh:
			log.Debug("new word receive", "req", req)
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				w.mu.RLock()
				w.current.beneficiary = w.coinbase
				if !w.chainConfig.IsErawan(w.current.header.Number) {
					w.current.beneficiary = w.sealer
				}
				w.mu.RUnlock()
...
func (w *worker) commitTransactions(env *environment, txs *types.TransactionsByPriceAndNonce, interrupt *atomic.Int32) error {
	...
	if env.gasPool == nil {
		...
		if w.chain.Config().IsChaophraya(env.header.Number) {
...
func (w *worker) commit(env *environment, interval func(), update bool, start time.Time) error {
	if w.isRunning() {
		...
		env := env.copy()
		block, _, err := w.engine.FinalizeAndAssemble(w.chain, types.CopyHeader(env.header), env.state, env.txs, env.unclelist(), env.receipts)
		if err != nil {
			return err
		}
		...
		if !w.isTTDReached(block.Header()) {
			...
			case w.taskCh <- &task{receipts: env.receipts, state: env.state, block: block, createdAt: time.Now()}:
				...
				// ! BKC PoS fee calculation does not relies on this process.
				// fees := totalFees(block, env.receipts)
				// feesInEther := new(big.Float).Quo(new(big.Float).SetInt(fees), big.NewFloat(params.Ether))
				log.Info("Commit new sealing work", "number", block.Number(), "sealhash", w.engine.SealHash(block.Header()),
					"uncles", len(env.uncles), "txs", env.tcount,
					"gas", block.GasUsed(), "fees", nil,
