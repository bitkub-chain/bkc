...
func (c *ChainConfig) Description() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v DAO: %v DAOSupport: %v EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Constantinople: %v Petersburg: %v Istanbul: %v, Erawan: %v, Chaophraya: %v, Muir Glacier: %v, Berlin: %v, London: %v, Arrow Glacier: %v}",
		c.ChainID,
		c.HomesteadBlock,
		c.DAOForkBlock,
		c.DAOForkSupport,
...
func (c *ChainConfig) CheckConfigForkOrder() error {
	...
	for _, cur := range []fork{
		...
		{name: "chaophrayaBangkokBlock", block: c.ChaophrayaBangkokBlock, optional: true},
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, headNumber *big.Int, headTimestamp uint64) *ConfigCompatError {
	...
	if isForkBlockIncompatible(c.ErawanBlock, newcfg.ErawanBlock, headNumber) {
		return newBlockCompatError("ErawanBlock fork block", c.ErawanBlock, newcfg.ErawanBlock)
	}
	if isForkBlockIncompatible(c.ChaophrayaBlock, newcfg.ChaophrayaBlock, headNumber) {
		return newBlockCompatError("ChaophrayaBlock fork block", c.ChaophrayaBlock, newcfg.ChaophrayaBlock)
	}
	if isForkBlockIncompatible(c.ChaophrayaBangkokBlock, newcfg.ChaophrayaBangkokBlock, headNumber) {
		return newBlockCompatError("ChaophrayaBangkokBlock fork block", c.ChaophrayaBangkokBlock, newcfg.ChaophrayaBangkokBlock)
	}
...
type Rules struct {
	...
	IsErawan, IsChaophraya, IsChaophrayaBangkok             bool
...
func (c *ChainConfig) Rules(num *big.Int, isMerge bool, timestamp uint64) Rules {
	...
	return Rules{
		...
		IsChaophrayaBangkok: c.IsChaophrayaBangkok(num),
